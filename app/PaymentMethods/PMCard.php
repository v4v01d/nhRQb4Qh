...
class PMCard extends PaymentMethod
{
    ...
    public function __construct(
        ...
        public readonly Carbon $cardExpirationDate
    ){
        ...
        $this->amount = 75.50;
        $config = config('app.payment_methods.bank_card');
    ...
    /**
     * Execute.
     * A payment method specific implementation.
     * Ideally a DTO should be returned instead of a regular array.
     */
    public function Execute():bool{
        $response = self::SendAPIRequest($this->endpoint, $this->method, $this->apiKey, [
            ...
            'card_exp_year' => $this->cardExpirationDate->year,
            'card_exp_month' => $this->cardExpirationDate->month,
            'cvv' => $this->cvv
        ]);
    ...
    private function SetResponse(Response $response):void{
        ...
        if($response->failed() && TRUE !== TRUE){
            ...
            Log::critical("Bank Card payment failed!", $this->responseErrors);
            return;
        }
